...
public class SimpleProject extends ProjectBase {
	static long getSessionId(ContentResolver resolver, long projectId) {
		...
		Cursor cursor = resolver.query(Sessions.CONTENT_URI, sessionsProjection, Sessions.PROJECT_ID + "=" + projectId,
				null, Sessions.DEFAULT_SORT_ORDER);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSessionPlayback = new SessionPlayback(savedInstanceState, this,
				ContentUris.withAppendedId(Sessions.CONTENT_URI, mSessionId));
		scrollToEndOfList();

		bindService(new Intent(IRecordService.class.getName()), mServiceConnection, Context.BIND_AUTO_CREATE);

		((ListView) findViewById(R.id.annotation_list)).getAdapter().registerDataSetObserver(new DataSetObserver() {
						...
						if (mRecordService != null)
							try {
								if (mRecordService.getState() == RecordService.State.RECORDING.ordinal()) {
									mCurrentTime.setText(mSessionPlayback.playTimeFormatter()
											.format(mRecordService.getTimeInRecording()));
