...
public class RehearsalData extends ContentProvider {
	...
	enum Project {
		_ID, TITLE, IDENTIFIER, TYPE
	}

	enum Session {
		_ID, PROJECT_ID, TITLE, IDENTIFIER, START_TIME, END_TIME
	}

	enum Annotation {
		_ID, RUN_ID, START_TIME, END_TIME, FILE_NAME
	}
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		void createProjectsTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + Projects.TABLE_NAME + "(" + Projects._ID + " INTEGER PRIMARY KEY,"
					+ Projects.TITLE + " TEXT," + Projects.IDENTIFIER + " TEXT," + Projects.TYPE + " INTEGER DEFAULT "
		...
		void createSessionsTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + Sessions.TABLE_NAME + "(" + Sessions._ID + " INTEGER PRIMARY KEY,"
					+ Sessions.PROJECT_ID + " INTEGER," + Sessions.TITLE + " TEXT," + Sessions.IDENTIFIER + " TEXT,"
					+ Sessions.START_TIME + " INTEGER," + Sessions.END_TIME + " INTEGER" + ");");
		}

		void createAnnotationsTable(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + Annotations.TABLE_NAME + "(" + Annotations._ID + " INTEGER PRIMARY KEY,"
					+ Annotations.SESSION_ID + " INTEGER," + Annotations.START_TIME + " INTEGER," + Annotations.END_TIME
					+ " INTEGER," + Annotations.FILE_NAME + " TEXT," + Annotations.VIEWED + " BOOLEAN DEFAULT FALSE,"
		...
		void upgrade5to6(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + AppData.TABLE_NAME + "(" + AppData._ID + " INTEGER PRIMARY KEY" + ");");
		}

		void upgrade6to7(SQLiteDatabase db) {
			db.execSQL(
					"ALTER TABLE " + Annotations.TABLE_NAME + " ADD COLUMN " + Annotations.LABEL + " TEXT DEFAULT ''");
		}

		void upgrade7to8(SQLiteDatabase db) {
			db.execSQL("ALTER TABLE " + Projects.TABLE_NAME + " ADD COLUMN " + Projects.TYPE + " INTEGER DEFAULT "
					+ Projects.TYPE_SESSION);
		}

		void upgrade8to9(SQLiteDatabase db) {
			// Display license if this is the first time running this version.
			String[] appDataProjection = { AppData._ID, AppData.KEY, AppData.VALUE };

			SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(mContext);
			SharedPreferences.Editor editor = preferences.edit();

			// get the current project information from AppData and move to
			// preferences
			Cursor project = db.query(AppData.TABLE_NAME, appDataProjection, AppData.KEY + "=" + "'current_project_id'",
					null, null, null, AppData.DEFAULT_SORT_ORDER, null);
			...
			Cursor visited_version = db.query(AppData.TABLE_NAME, appDataProjection,
					AppData.KEY + "=" + "'app_visited_version'", null, null, null, AppData.DEFAULT_SORT_ORDER, null);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case SESSIONS:
			Cursor c = db.query(Sessions.TABLE_NAME, new String[] { Sessions._ID }, selection, selectionArgs, null,
					null, null);
			count = 0;
		...
		case SESSION_ID:
			...
			count = db.delete(Sessions.TABLE_NAME,
					Sessions._ID + "=" + sessionId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			if (count > 0)
		...
		case PROJECT_ID:
			...
			count = db.delete(Projects.TABLE_NAME,
					Projects._ID + "=" + projectId + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			if (count > 0)
	...
	int deleteAnnotations(SQLiteDatabase db, String selection, String[] selectionArgs) {
		...
		String[] projection = { Annotations.FILE_NAME };
