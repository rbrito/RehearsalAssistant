...
public class AppDataAccess {
	...
	long getProjectIdNot(long id) {
		Cursor projectsCursor = mContext.getContentResolver().query(Projects.CONTENT_URI, projectsDataProjection,
				Projects._ID + "<>" + id, null, Projects.DEFAULT_SORT_ORDER);
	...
	long getFirstProjectID() {
		Cursor c = mContext.getContentResolver().query(Projects.CONTENT_URI, projectsDataProjection, null, null,
				Projects.DEFAULT_SORT_ORDER);
		c.moveToFirst();
	...
	private long getRecorderWidgetProjectIdImpl(boolean createIfNeeded) {
		...
		String[] projectsProjection = { Projects._ID, Projects.TYPE };

		Cursor projectsCursor;

		// if there was a projectId in preferences, make sure it exists and is
		if (projectId != -1) {
			projectsCursor = mContext.getContentResolver().query(
					ContentUris.withAppendedId(Projects.CONTENT_URI, projectId), projectsProjection, null, null,
					Projects.DEFAULT_SORT_ORDER);
			if (projectsCursor.getCount() > 0) {
				projectsCursor.moveToFirst();
		...
		projectsCursor = mContext.getContentResolver().query(Projects.CONTENT_URI, projectsProjection,
				Projects.TYPE + "=" + Projects.TYPE_SIMPLE, null, Projects.DEFAULT_SORT_ORDER);
		if (projectsCursor.getCount() == 0) {
			if (!createIfNeeded)
				return -1;
			mContext.getContentResolver().insert(Rehearsal.Projects.CONTENT_URI,
					RehearsalData.valuesForMemoProject(mContext));
	...
	static String[] projectsDataProjection = { Projects._ID, };
