...
public class ProjectBase extends RehearsalActivity {
	public void onCreate(Bundle savedInstanceState) {
		...
		String[] projectsProjection = { Projects._ID, Projects.TITLE };

		Cursor projectCursor = getContentResolver().query(ContentUris.withAppendedId(Projects.CONTENT_URI, mProjectId),
		...
		if (visitedVersion < 0.5f) {
			Request.notification(this, this.getResources().getString(R.string.warning),
					getString(R.string.beta_warning));
			Request.confirmation(this, getString(R.string.license2), getString(R.string.license),
			...
			Request.notification(this, this.getString(R.string.uncompressed_recording),
					this.getString(R.string.uncompressed_recording2));
			mAppData.setVisitedVersion(RehearsalAssistant.currentVersion);
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (!super.onOptionsItemSelected(item)) {
			if (item == mSwitchMenuItem) {
				startActivity(new Intent(Intent.ACTION_VIEW, Projects.CONTENT_URI));
	...
	protected static final String[] sessionsProjection = new String[] { Sessions._ID, // 0
			Sessions.TITLE, // 1
